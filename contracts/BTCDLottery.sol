...
// File: contracts/interfaces/IBTCDRandomGenerator.sol

pragma solidity ^0.8.4;

interface IBTCDRandomGenerator {
    /**
...
// File: contracts/interfaces/IBTCDLottery.sol

pragma solidity ^0.8.4;

interface IBTCDLottery {
    /**
    ...
    /**
     * @notice Draw the final number, calculate reward in BTCD per group, and make lottery claimable
     * @param _lotteryId: lottery id
     ...
     * @notice Inject funds
     * @param _lotteryId: lottery id
     * @param _amount: amount to inject in BTCD token
     * @dev Callable by operator
     */
     ...
     * @param _priceTicketInBTCD: price of a ticket in BTCD
...
// File: contracts/BTCDLottery.sol

pragma solidity ^0.8.4;
pragma abicoder v2;

/** @title Bitdollars Lottery.
 * @notice It is a contract for a lottery system using
 * randomness provided externally.
 */
contract BTCDLottery is ReentrancyGuard, IBTCDLottery, Ownable {
    ...
    uint256 public maxPriceTicketInBTCD = 50 ether;
    uint256 public minPriceTicketInBTCD = 0.005 ether;
    ...
    IERC20 public btcdToken;
    IBTCDRandomGenerator public randomGenerator;
    ...
    struct Lottery {
        ...
        uint256 startTime;
        uint256 endTime;
        uint256 priceTicketInBTCD;
        uint256 discountDivisor;
        uint256[6] rewardsBreakdown; // 0: 1 matching number // 5: 6 matching numbers
        uint256 treasuryFee; // 500: 5% // 200: 2% // 50: 0.5%
        uint256[6] btcdPerBracket;
        uint256[6] countWinnersPerBracket;
        uint256 firstTicketId;
        uint256 firstTicketIdNextLottery;
        uint256 amountCollectedInBTCD;
        uint32 finalNumber;
    }
    ...
    /**
     * @notice Constructor
     * @dev BTCDRandomGenerator must be deployed prior to this contract
     * @param _btcdTokenAddress: address of the BTCD token
     * @param _randomGeneratorAddress: address of the RandomGenerator contract used to work with ChainLink VRF
     */
    constructor(address _btcdTokenAddress, address _randomGeneratorAddress) {
        btcdToken = IERC20(_btcdTokenAddress);
        randomGenerator = IBTCDRandomGenerator(_randomGeneratorAddress);
        ...
        nonReentrant
    {
        ...
        // Calculate number of BTCD to this contract
        uint256 amountBTCDToTransfer = _calculateTotalPriceForBulkTickets(
            _lotteries[_lotteryId].discountDivisor,
            _lotteries[_lotteryId].priceTicketInBTCD,
            _ticketNumbers.length
        );

        // Transfer BTCD tokens to this contract
        uint256 balanceBefore = btcdToken.balanceOf(address(this));
        btcdToken.safeTransferFrom(
            address(msg.sender),
            address(this),
            amountBTCDToTransfer
        );
        amountBTCDToTransfer = btcdToken.balanceOf(address(this)).sub(
            balanceBefore
        );

        // Increment the total amount collected for the lottery round
        _lotteries[_lotteryId].amountCollectedInBTCD = _lotteries[_lotteryId]
            .amountCollectedInBTCD
            .add(amountBTCDToTransfer);
    ...
    function claimTickets(
        ...
        // Initializes the rewardInBTCDToTransfer
        uint256 rewardInBTCDToTransfer;
        ...
        for (uint256 i = 0; i < _ticketIds.length; i++) {
            ...
            rewardInBTCDToTransfer = rewardInBTCDToTransfer.add(
                rewardForTicketId
            );
        }

        // Transfer money to msg.sender
        btcdToken.safeTransfer(msg.sender, rewardInBTCDToTransfer);

        emit TicketsClaim(
            msg.sender,
            rewardInBTCDToTransfer,
            _lotteryId,
            _ticketIds.length
        );
    }
    ...
    function drawFinalNumberAndMakeLotteryClaimable(
        ...
        // Calculate prizes in BTCD for each bracket by starting from the highest one
        for (uint32 i = 0; i < 6; i++) {
            ...
            if (
                ...
                if (_lotteries[_lotteryId].rewardsBreakdown[j] != 0) {
                    _lotteries[_lotteryId].btcdPerBracket[j] =
                ...
                // A. No BTCD to distribute, they are added to the amount to withdraw to treasury address
            } else {
                _lotteries[_lotteryId].btcdPerBracket[j] = 0;
        ...
        amountToWithdrawToTreasury = amountToWithdrawToTreasury
            .add(_lotteries[_lotteryId].amountCollectedInBTCD)
            .sub(amountToShareToWinners);

        // Transfer BTCD to treasury address
        if (amountToWithdrawToTreasury > 0) {
            btcdToken.safeTransfer(
                treasuryAddress,
        ...
        onlyOwner
    {
        ...
        // Request a random number from the generator based on a seed
        IBTCDRandomGenerator(_randomGeneratorAddress).getRandomNumber(
            uint256(
        ...
        // Calculate the finalNumber based on the randomResult generated by ChainLink's fallback
        IBTCDRandomGenerator(_randomGeneratorAddress).viewRandomResult();
        ...
        onlyOwnerOrInjector
    {
        ...
        _amount = btcdToken.balanceOf(address(this)).sub(balanceBefore);
    ...
    function startLottery(
        ...
        require(
            (_priceTicketInBTCD >= minPriceTicketInBTCD) &&
                (_priceTicketInBTCD <= maxPriceTicketInBTCD),
            "Outside of limits"
        );
        ...
        _lotteries[currentLotteryId] = Lottery({
            ...
            endTime: _endTime,
            priceTicketInBTCD: _priceTicketInBTCD,
            ...
            amountCollectedInBTCD: pendingInjectionNextLottery,
            finalNumber: 0
        });
    ...
    function recoverWrongTokens(address _tokenAddress, uint256 _tokenAmount)
        ...
        onlyOwner
    {
        require(_tokenAddress != address(btcdToken), "Cannot be BTCD token");
    ...
    /**
     * @notice Set BTCD price ticket upper/lower limit
     * @dev Only callable by owner
     * @param _minPriceTicketInBTCD: minimum price of a ticket in BTCD
     * @param _maxPriceTicketInBTCD: maximum price of a ticket in BTCD
     */
    function setMinAndMaxTicketPriceInBTCD(
        uint256 _minPriceTicketInBTCD,
        uint256 _maxPriceTicketInBTCD
    ) external onlyOwner {
        require(
            _minPriceTicketInBTCD <= _maxPriceTicketInBTCD,
            "minPrice must be < maxPrice"
        );

        minPriceTicketInBTCD = _minPriceTicketInBTCD;
        maxPriceTicketInBTCD = _maxPriceTicketInBTCD;
    }
     ...
     * @param _priceTicket price of a ticket (in BTCD)
    ...
    function _calculateRewardsForTicketId(
        ...
        if (transformedWinningNumber == transformedUserNumber) {
            return _lotteries[_lotteryId].btcdPerBracket[_bracket];
        } else {
            return 0;
        }
    }
